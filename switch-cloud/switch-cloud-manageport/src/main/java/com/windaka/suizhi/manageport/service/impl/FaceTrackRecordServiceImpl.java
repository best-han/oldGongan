	...
	public void saveFaceTrackRecord(String xqCode, List<Map<String, Object>> params) throws OssRenderException, IOException {
		...
		for (int i = 0; i < params.size(); i++) {
			...
			if (null != params.get(i).get("base64Img") && StringUtils.isNotBlank(params.get(i).get("base64Img").toString())) {
				String base64Img = params.get(i).get("base64Img").toString();
				// 获取最大id值
				Integer maxId = faceTrackRecordDao.getMaxRecordId();
				String fileName = PicUtil.getPicName("face_track_record", maxId==null?1:(maxId + 1));
				byte[] byteArr = PicUtil.stringToInputStream(base64Img);
				// 图片放入打包路径
			...
			faceTrackRecordDao.saveFaceTrackRecord(param);
			param.put("xqCode",xqCode);
			String sqlStr= SqlCreateUtil.getSqlByMybatis(FaceTrackRecordDao.class.getName()+".saveFaceTrackRecord",param);
